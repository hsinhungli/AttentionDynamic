...
p.T             = 10*1000;    %total duration to simulate (ms)
p.nt            = p.T/p.dt+1;
p.tlist         = 0:p.dt:p.T;

%Temporal dynamic of neurons
p.tau             = [10 10 10 10 10];           %time constant (ms)
p.tau_a           = [1200 1200 1200 1200 1200]; %time constant adaptation (ms)
p.tau_inh         = 5;
p.tau_att         = 200;                      %time constant attention (ms)
p.tau_n           = 100;                      %time constant noise (ms)
p.d_noiseamp      = [0.001 0.001 0 0 0]*0;    %noise amplitude of each layer (implemented at excitatory drive; set to zero for noise-free system)
...
p.baselineAtt   = 1;
p.p             = [2 2 2 2 2];      %exponentiation of each layer
