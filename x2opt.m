...
%% initial param vals
% sensory
p.tau           = 50;  %70, 20         %time constant filter for excitatory response (ms)
% p.tauwm         = 20;              %time constant filter for working memory (ms)
p.tau_r2        = 75;  %100            %time constant filter for firing rate (ms)
p.sigma         = 2; % .5, .1         %semisaturation constant
% p.p             = 1.5;
...
p.attOffset     = 80;              % voluntary attention off, relative to stim offset (ms)
...
% p.gam1          = 8;
% p.gam2          = .005;
p.aMI           = 3; % 5 (spatial sim), 4 (stronger IOR), 4 (temporal sim)
p.aIOR          = .7; % 1 (spatial sim), 1.3 (stronger IOR), 1.12 (temporal sim)
% p.asigma        = .3;  %.3
